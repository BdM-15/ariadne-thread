...
- Web Source Collection: a bounded public-source collection step inside Capture Research Enrichment, such as Firecrawl-backed fetching, that records source URL, source type, collection time, access limits, and capability provenance.
- Live Source Collection Run: a user-approved or policy-approved execution of a real external source-collection capability, such as Firecrawl, that may spend credits or call live services and must record scope, source targets, approval basis, collection time, and provenance.
...
- Web Enrichment Support: the web-source collection lane within Capture Research Enrichment, such as Firecrawl-backed discovery over accessible public customer, competitor, partner, news, article, or agency pages when official identifiers are missing, program names changed, descriptions are fuzzy, or context outside deterministic data sources is needed.
...
- A **SAM.gov Enrichment Profile** can recommend **Capture Research Enrichment** when official SAM.gov signals are ambiguous, stale, renamed, or incomplete; Firecrawl-backed **Web Enrichment Support** is one possible source-collection lane inside that broader research workflow.
...
- Future autonomous or Hermes-coordinated **Live Source Collection Runs** should follow **Graduated Autonomy** rules, such as approved source scope, budget or credit boundaries, reviewability, and reversible downstream effects, rather than running unbounded external research.
...
- "web enrichment" was too narrow for the next research workflow — resolved: use **Capture Research Enrichment** for the broader workflow that combines deterministic data, planned research, web source collection, and reviewable interpretation; keep **Web Enrichment Support** as the Firecrawl-style source collection lane.
