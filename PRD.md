...
- Project Ariadne public-source knowledge currently exists under `docs/reference/project-ariadne/knowledge/` as a temporary import corpus for MVP-1C. It should be fully transformed into Ariadne-native LLM-wiki pages, source summaries, data-element context, relationship maps, artifact/data-element expectations, and reusable insight candidates in the canonical vault. The vestigial `docs/reference/project-ariadne/` folder should be retired only after maintainer review confirms the vault coverage is adequate; deletion is not automatic.
...
- MVP-1B is complete: Opportunity Activation + Packet Field Action Matrix + Opportunity Portfolio Foundation reached the route-first, review-gated stopping point on `12-build/opportunity-activation-field-matrix`. The selected next slice is MVP-1C: **LLM-Wiki Knowledge Vault Foundation**. Do not start unrelated deferred artifact, renderer, RAG engine, graph database, parser, external API, broad UI migration, or broad Hermes autonomy work while implementing this bounded vault slice.
...
**MVP-1C: LLM-Wiki Knowledge Vault Foundation** ← **SELECTED NEXT**
...
**The next build direction is MVP-1C: LLM-Wiki Knowledge Vault Foundation. Build one canonical Ariadne vault at `knowledge/ariadnes-thread/`, fully migrate the Project Ariadne knowledge vestige before review-gated retirement, and use the vault's global data elements and typed relationships to improve Opportunity Activation and field-level execution before moving into real AI/skills/MCP execution behind those routes.**
