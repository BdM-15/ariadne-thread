...
- SAM.gov Enrichment Profile implementation now includes the Entity Record lane, Opportunity Discovery lane, and Known Opportunity Record lane on the SAM.gov epic branch, with live/fake source-mode boundaries, local profile persistence, API routes, Command Center read surfaces, source limitations, and review-gated downstream candidates.
- Current automated validation: `uv run ruff check src tests` and `uv run pytest -q` pass on the SAM.gov Known Opportunity Record branch, with 184 tests passing.
