...
- Live validation is a separate local/manual check when the relevant local service or private key is configured: inspect `GET /api/capture-research/source-providers`, run `POST /api/capture-research/source-providers/{provider_id}/smoke-check` with explicit approval for each configured provider, then create a bounded Capture Research Run and call `POST /api/capture-research/runs/{research_run_id}/source-provider-collection` with explicit approval.
...
- #58 added reviewable downstream candidate projection and review decisions for Capture Research runs. Source Findings and generated insight candidates now project into grouped Evidence, Packet, Action Plan, Risk Register, Call Plan, Follow-Up Route, Price/Workload Assumptions, Teaming Partner Needs, and BCC-Ready Notes candidates; accept, discard, and route decisions preserve run/brief/trigger/source/lens/baseline provenance while avoiding automatic trusted downstream writes.
- #59 prepared the full Capture Research Command Surface review shape in the existing FastAPI Command Center scaffold. The surface now shows live source readiness, the Capture Research Brief, trigger context, source-profile refs, collection provenance, Source Findings, selected lenses, Seller Capability Baseline refs, Requirements Fit, Competitive Gap, selected lens analyses, Research Summary View, grouped downstream candidates, candidate review actions, recorded review decisions, and related Ariadne record links. The issue remains open until the user reviews or explicitly defers the first UI shape.
