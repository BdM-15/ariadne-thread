...
```bash
# Install the CLI Hub package manager as a uv-managed developer tool
...
`cli-hub` is a lightweight package manager for CLI-Anything harnesses. When you run `cli-hub install gimp`, it installs a separate Python package (`cli-anything-gimp`) with its own CLI entry point (`cli-anything-gimp`). Each CLI is an independent package — `cli-hub` resolves names from the registry and tracks installs. In Ariadne, install `cli-hub` itself with `uv tool install cli-anything-hub`.
