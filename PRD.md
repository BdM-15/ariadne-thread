...
**Product Requirements Document (PRD) v1.65**
...
- Project Ariadne public-source knowledge currently exists under `docs/reference/project-ariadne/knowledge/` as a temporary import corpus for MVP-1C. It should be transformed into Ariadne-native LLM-wiki pages, source summaries, data-element context, relationship maps, and reusable insight candidates in the canonical vault, then the vestigial `docs/reference/project-ariadne/` folder should be deleted rather than maintained as a parallel knowledge home.
...
- Define Ariadne page types and frontmatter for global data elements, capture concepts, source summaries, entities, relationships, workflow/capability pages, reusable insight candidates, opportunity projections, and Hermes learning proposals.
- Define a typed relationship vocabulary for lightweight knowledge graph links such as supports, answers, informs, blocks, contradicts, derived from, evidence for, fills gap in, suggests route, uses capability, applies to gate, produces artifact block, and candidate reusable insight.
...
- Migrate useful material from `docs/reference/project-ariadne/knowledge/` into Ariadne-native vault pages, source summaries, data-element context, relationship maps, and reusable insight candidates. Preserve provenance in frontmatter and a migration coverage report, then delete the vestigial `docs/reference/project-ariadne/` folder after enough material has been incorporated.
...
- The migration coverage report shows what Project Ariadne material was incorporated, skipped, or still pending, and states that `docs/reference/project-ariadne/` is temporary until migration completion.
...
**The next build direction is MVP-1C: LLM-Wiki Knowledge Vault Foundation. Build one canonical Ariadne vault at `knowledge/ariadnes-thread/`, migrate and retire the Project Ariadne knowledge vestige, and use the vault's global data elements and typed relationships to improve Opportunity Activation and field-level execution before moving into real AI/skills/MCP execution behind those routes.**
