...
- Capture Work Product Loop: the review-gated workflow that turns routed AI, skill, research, source-profile, or capability outputs into visible improvements to concrete capture work products.
- Work Product Delta: a reviewable proposed change to one or more concrete capture work products, carrying before and after meaning, intended destination, source support, capability provenance, assumptions, gaps, and review state.
...
- A **Capture Work Product Loop** should start from a reviewed or review-ready routed output, create one or more **Work Product Deltas**, and show the operator how concrete work products would change before trusted records are updated.
- MVP-3 first tracer is implemented: review-ready `competitive-gap-route-hint` output creates packet/action deltas, review decisions apply destination rules, engagement-prep candidates are review-gated, and artifact source-package freshness updates after accepted packet/action deltas.
- MVP-3 acceptance demo is scripted in `docs/architecture/mvp-3-capture-work-product-loop-demo.md` and validated through production command-center tests plus Next.js typecheck.
- MVP-3 UI/workflow shape sign-off is complete (issue `#107`), and MVP-3 first tracer is accepted as current stage baseline.
- A **Work Product Delta** can target the **Living Milestone Decision Briefing Packet**, **Capture Action Plan**, **Call Plan**, **Risk Register**, follow-up routes, or **Artifact Draft** context, but it should update only the intended destination after explicit review.
- Accepting a **Work Product Delta** should use the destination work product's existing review rules; discarding or routing a delta should preserve provenance without creating trusted downstream records.
