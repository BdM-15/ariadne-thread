...
**Product Requirements Document (PRD) v1.72**
...
**Date:** May 22, 2026
**Status:** Artifact Assembly Foundation complete; production Command Center UI/UX plan incorporated; MVP-1 production Command Center tracer is on review branch with the first Opportunity Intake and Portfolio Foundation loop; MVP-1B Opportunity Activation + Packet Field Action Matrix is complete with a deterministic local-first activation run seam, a Next.js Autonomy Digest and Packet Field Action Matrix surface, explicit review-gated Packet Field Answer promotion paths, a selected Command Center Work Modes IA correction so the home surface stays a pulse/router instead of a mega-scroll page, clarified Milestone 1-4 gate discipline with the Living Milestone Decision Briefing Packet as the primary opportunity roadmap, first-class Milestone Gate status in the Opportunity scaffold/portfolio/workspace contract, a stage-relative, gate-scoped packet roadmap model, Packet roadmap and Activation Matrix cards that launch field-specific Assisted Capture routes for missing data elements, accepted field-specific route outputs that promote opportunity-scoped Packet Field Answers, Assisted Capture UI feedback that shows the created Packet Answer after review acceptance, material-refresh activation runs that reconcile the Packet roadmap after accepted field route output, portfolio/pulse attention links that deep-link directly into the focused route or field-specific Assisted Capture loop, a first Living Packet Live View that visually connects packet sections, data elements, field states, source support, and action routes, Assisted Capture route cards that expose Packet Field Route Kind through recommendation and run output review, first focused Actions, Engagement, Research, Documents, and Capability Studio Work Modes fed by opportunity-scoped route projections, Capture Research runs, source-provider readiness, Document Intake queue/draft/candidate/capability readiness, Capability Run history/output/provenance/catalog readiness, and packet-field route gaps, a local-first portfolio status/update API plus visible create/select portfolio status controls for future, watchlist, active, held, archived, won, and lost Opportunity management, explicit Portfolio API/UI urgency and source-freshness labels for global pulse triage, explicit Packet Field Route Kinds proving source-backed answer, research/MCP candidate, source-profile lookup, model-synthesis, and customer-call-plan fallback routes, route-kind-specific deterministic Assisted Capture outputs with approval provenance, and Activation Matrix route steps/approval gates that expose answer paths, route rationale, approvals needed, and review boundaries before any live capability execution. MVP-1C: LLM-Wiki Knowledge Vault Foundation is complete and merged to `main`: canonical vault scaffold, Ariadne-native global data elements, typed relationships, full Project Ariadne migration with review-gated retained corpus, Mirror Update Proposal guardrail, vault health, grouped data dictionaries, and vault-informed route recommendations are in place. MVP-2: AI Usage Layer + Skills Integration is complete and merged to `main`: explicit model-use contracts, guarded Local Admin Model and hosted reasoning readiness routes, seven focused workspace skills, deterministic thin chain maps, and Opportunity Activation route cards for source-backed, source-profile, model, call-plan, and skill-chain paths are in place. MVP-3: Capture Work Product Loop first tracer is complete (`#102`-`#106`) with packet/action/engagement/artifact-context review-gated loop.

---

## 0. Current State Snapshot (May 21, 2026)
...
- Project Ariadne public-source knowledge has been migrated from `docs/reference/project-ariadne/knowledge/` into Ariadne-native LLM-wiki pages, source summaries, data-element context, relationship maps, artifact/data-element expectations, and reusable insight candidates in the canonical vault. The vestigial `docs/reference/project-ariadne/` folder remains retained reference material for now; deletion stays deferred until separate explicit approval.
...
- The next MVP branch (`12-build/opportunity-activation-field-matrix`) now includes a deterministic Opportunity Activation Run module, local activation-run store, Packet Field Action Matrix model, Autonomy Digest generation from packet-field definitions and answers, create-time initial activation storage, production Command Center API routes to list or request activation runs, and a Next.js Opportunity Activation panel that shows latest-run coverage, blocked fields, review-ready counts, approvals, source limits, next-best actions, and field route cards from inside the main workspace. The branch also adds explicit field review decision paths: accepting or editing a matrix field creates an opportunity-scoped Packet Field Answer with provenance, and accepting a field-specific Assisted Capture route output also promotes a Packet Field Answer tied to the selected data element; the Assisted Capture panel now sends accepted outputs to their actual recommended destination, visibly confirms created Packet Answers after review, refreshes the Command Center, and Packet mode keeps accepted answers visible in a Supported Answers lane after a material-refresh activation run reconciles the roadmap. Portfolio and Global Opportunity Pulse attention cards now deep-link to the focused mode or directly into field-specific Assisted Capture when a packet field is the next route, and Activation Matrix field cards now expose the same direct assisted-route launch before any manual review decision is recorded. Route and discard decisions only record review state. It now makes Milestone 1-4 Gate status first-class in Opportunity creation, scaffold persistence, portfolio pulse, and workspace display, Packet Field Definitions/Action Matrix rows carry gate scope so the Packet roadmap shows data elements required for the current milestone before future-gate fields, Packet roadmap cards can start a field-specific Assisted Capture route for the selected data element, the production API exposes explicit portfolio status plus update/archive behavior for future, watchlist, active, held, archived, won, and lost Opportunity management, New Opportunity can assign initial portfolio status, and the Command Center left rail plus Global Opportunity Pulse now show visible portfolio status chips/groups so active, future/watchlist, held, and past/archive Opportunities are switchable in the main workflow. Packet Field Route Kinds now distinguish source-backed answers, research/MCP candidates, source-profile lookup, model synthesis, and customer-call-plan fallback; tests prove source-backed acceptance with evidence refs, research-backed field routes, and call-plan fallback that improves work products without prematurely promoting a Packet Field Answer. Packet Mode now includes a first Living Packet Live View that groups current-gate fields by packet section and shows each field's answer state, source count, route kind, and direct route action as a visual packet surface, Assisted Capture route cards now show route kind chips before and after route execution so the user can see whether a path is source-backed, research/MCP, source-profile, model-synthesis, or customer-call-plan fallback, Actions mode now shows selected-opportunity Action Plan route projections plus packet gaps that can become next tasks, Engagement mode now shows selected-opportunity call-plan projections plus customer-input gaps from the same route spine, Research mode now shows selected-opportunity Capture Research runs, source-provider readiness, review-candidate counts, and research-routed packet gaps, Documents mode now shows Document Intake queue records, extraction draft parts, review-gated capture candidates, parser capability gaps, and source-backed packet gaps, and Capability Studio mode now shows Capability Run history, pending outputs, validation gaps, local catalog entries, and latest run provenance without launching new automation. The current IA correction is to keep the Command Center Home as a pulse-and-routing cockpit, then move detailed action surfaces into focused Work Modes so real Opportunity management does not become one huge scrolling page. This slice remains deliberately review-gated and does not create trusted Evidence, Action Plan Items, or downstream work products automatically.
- The same branch now feeds the Command Center left-rail badges from actual review queues for Documents and Capability Studio: Document counts include parser-required/ungenerated intake records, pending extraction bundle review, and pending document-derived capture candidates for the selected Opportunity, while Capability Studio counts pending Capability Run Outputs across the local run store. Capability Studio is now part of the explicit work-mode order instead of an appended extra mode, keeping advanced capability review visible while still behind the product workflow surface.
- Assisted Capture route execution now specializes deterministic reviewable outputs by Packet Field Route Kind: source-backed answer, research/MCP, source-profile lookup, model synthesis, and customer-call-plan fallback each produce distinct titles, summaries, assumptions, gaps, and approval provenance while preserving the no-network/no-model/no-trusted-write boundary.
- Packet Field Action Matrix rows now expose deterministic route steps and approval gates, while the Autonomy Digest shows approval-required routes alongside coverage, source limits, skill chains, and next-best actions. This keeps skill/capability/source-provider hints visible inside the Opportunity Activation workflow without launching live capabilities or writing trusted answers automatically.
...
- MVP-1C is complete: **LLM-Wiki Knowledge Vault Foundation** created the canonical Ariadne Obsidian/LLM-wiki vault at `knowledge/ariadnes-thread/`, used Karpathy-style `index.md`, `log.md`, schema instructions, pages, and lint/health workflows, made global data elements and typed relationships the lightweight knowledge graph that improves opportunity execution, and kept opportunity-specific Packet Field Answers, Evidence Items, Action Plan state, review decisions, and trusted workflow records in Ariadne's structured stores.
- MVP-1C data-element projection treats the code-owned Briefing Packet field definitions, the Risk Register data dictionary, and the Call Plan data dictionary as explicit vault dictionary groups under `data-elements/briefing-packet/`, `data-elements/risk-register/`, and `data-elements/call-plan/`, with a shared `data-elements/dictionary-index.md` and relationship refs that point to the grouped paths instead of old root-level packet pages.
- MVP-1C completion validation on `main`: `uv run ruff check src tests` passed and `uv run pytest -q` passed with 349 tests after merge `d8ea9ff`.
- MVP-2 is complete and merged to `main`: AI Usage Layer + Skills Integration now includes explicit model-use contracts for seven PRD roles, guarded Local Admin Model and hosted reasoning readiness Capability Runs, `HOSTED_REASONING_MODEL_ENABLED=false` as the safe default, seven focused review-gated workspace skills, deterministic thin chain maps, Opportunity Activation route cards for all current Packet Field Route Kinds, and PRD-aligned review boundaries. Completion validation: `uv run ruff check src tests` passed and `uv run pytest -q` passed with 402 tests.

**Still Deferred**

- Hermes runtime beyond vault-maintenance proposals, semantic retrieval or RAG engine, graph databases, full graph visualization beyond Obsidian's file-link graph, full MinerU integration, RAGAnything integration, LightRAG integration, Theseus solicitation parser integration, OCR/multimodal extraction, final huashu-design/artifact rendering adapters, external API integrations beyond completed SAM.gov and the selected Capture Research Enrichment source-provider lane, additional third-party skill installation beyond the vendored marketing skills, persisted vector/graph storage, persistent storage beyond local/demo or narrow workflow adapters, full autonomous Opportunity Activation Runs, trusted field promotion beyond the first Packet Field Answer accept/edit path, full multi-opportunity lifecycle management, and the full production Knowledge Vault UI are not implemented yet. MVP-1 now has a production-shaped Next.js route tracer and first portfolio selector, MVP-1B has a deterministic activation-run seam plus first visible field matrix surface and field-review controls, and MVP-2 has guarded model/skill Capability Runs; broad UI migration, full portfolio workflows, trusted field-level automation, live external execution, and autonomous capability execution remain governed by the MVP roadmap. Work Modes may expose placeholders for focused surfaces only when they honestly show current implementation status.
...
- MVP-2 is complete: AI Usage Layer + Skills Integration reached the agreed review-gated stopping point on `main`. The selected next slice is MVP-3: **Capture Work Product Loop**. Do not start unrelated broad UI hardening, final renderer/export, solicitation parser/RAG/graph/Hermes runtime, external API expansion, or automatic trusted-write work while implementing this bounded work-product loop.
- MVP-3 starts from the completed Opportunity Activation, Packet Field Action Matrix, Capability Run Store, AI/model contracts, focused skills, federal-data/source-provider capabilities, Artifact Assembly Source Packages, and LLM-wiki Knowledge Vault.
- MVP-3 first tracer is selected in `docs/architecture/mvp-3-capture-work-product-loop-plan.md`: start packet-first from a review-ready `competitive-gap-route-hint` output, create Work Product Deltas, accept or route each delta through explicit review, update the Living Packet via Packet Field Answer rules, project linked Action Plan implications through the recommendation gate, and refresh artifact source-package context without renderer/export work.
- MVP-3 must prove routed outputs improve concrete work products: the Living Milestone Decision Briefing Packet, Capture Action Plan, call/engagement prep, risk/follow-up candidates, and refreshed Artifact Draft context.
...
- Preserve completed boundaries: upstream federal-data MCPs stay behind Federal Data Capabilities, downloaded source material enters Document Intake, Capability Run Outputs land in review, the LLM-wiki vault can own global synthesis and relationship maintenance but not opportunity execution truth, Structured Knowledge Index remains the structured relationship baseline until a later storage ADR changes it, and trusted downstream writes remain human-gated.
...
| Obsidian / LLM-Wiki Knowledge Vault           | Canonical local Ariadne second brain for global data elements, capture concepts, source summaries, typed relationships, reusable insights, Hermes learning proposals, and Obsidian graph browsing. It uses Ariadne schema conventions and informs execution without becoming opportunity-specific source-of-truth state. | https://github.com/kepano/obsidian-skills and Karpathy LLM-wiki pattern |
...
- **Storage**: Local-first structured Evidence Store, narrow workflow stores such as the Document Intake Store, and file system with optional encrypted sync. The canonical local LLM-wiki vault lives at `knowledge/ariadnes-thread/` by default and is configured through `ARIADNE_OBSIDIAN_VAULT_DIR`; it is the compounding human/LLM-facing knowledge workspace for global concepts and relationships, while opportunity-specific execution state stays in Ariadne's structured stores.
...
- **Living Packet Live View** as the packet workspace surface: a visual decision-brief representation of the packet itself, where packet sections expose their required data elements, each data element shows answer/gap/source/review state, and the user can see which actions, routes, reviews, or source refs filled or still block the item.
...
The next polished packet UI should not reduce this to a table. It should render the Living Packet as a live visual packet surface: section blocks, field-status overlays, source/evidence chips, action traces, and focused drawers that show how a data element was filled or what route will fill it next. The visual packet should be the user's mental model; the Packet Field Action Matrix is the underlying control system.
...
The Knowledge Layer should first use the LLM-wiki Knowledge Vault as the lightweight knowledge graph surface for global capture concepts, data elements, source summaries, reusable insights, skill/capability relationships, and typed links between information elements. Obsidian's backlinks and graph view are the first practical Graph Sensemaking Mode over the file-based vault. Ariadne's structured relationship model still governs opportunity-specific execution state, and a later Knowledge Graph View can add product-native Graph Action Mode so selected nodes can create actions, suggest Insight Promotion, generate briefing sections, or launch research workflows.

If the Obsidian/LLM-wiki vault is edited directly, global concept and relationship pages can evolve in the vault according to the Ariadne wiki schema. Any edit that would change opportunity-specific Packet Field Answers, Evidence Items, Action Plan state, review decisions, source spans, artifact block reviews, autonomy permissions, or other trusted workflow records must return to Ariadne as a Mirror Update Proposal rather than directly overwriting structured knowledge. Ariadne should classify, validate, and route those proposals through the same Knowledge Processing Workflow used by Quick Capture so traceability and source-of-truth discipline are preserved.
...
4. Use accepted Ariadne knowledge, Capture Reference Context, and the LLM-wiki Knowledge Vault as the first Seller Capability Baseline, with temporary Project Ariadne material flowing through the MVP-1C migration path rather than remaining a separate reference home.
...
6. As a capture professional, I want the seller baseline to draw from accepted Ariadne knowledge and the LLM-wiki Knowledge Vault, so that migrated reference knowledge becomes useful in active opportunity work without preserving a separate Project Ariadne knowledge home.
...
- browse and reuse the Ariadne LLM-wiki Knowledge Vault: global data elements, capture concepts, source summaries, typed relationships, skills/capability context, accepted evidence projections, source material summaries, lessons, reusable insights, and opportunity-scoped links with clear authority and opportunity scope.
...
12. **Knowledge vault capture**: accepted knowledge, reusable insights, lessons, source refs, global data-element context, typed relationships, and reviewed capability outputs become discoverable in the Ariadne LLM-wiki Knowledge Vault without confusing wiki synthesis for opportunity-specific source-of-truth records.
...
- **Ariadne LLM-Wiki Knowledge Vault** is the canonical local Obsidian-compatible second brain at `knowledge/ariadnes-thread/`. It owns global capture concepts, data-element pages, source summaries, skill/capability context, reusable insight candidates, relationship maps, `index.md`, `log.md`, health/lint reports, and Hermes learning proposals using Ariadne schema conventions. It informs opportunity execution through typed links and source refs, while opportunity-specific Packet Field Answers, Evidence Items, Action Plan state, review decisions, source spans, artifact block reviews, and trusted workflow records remain in Ariadne's structured stores.
...
- **Living Packet Live View** is the user-facing visual packet workspace built from the Packet Field Action Matrix and Packet Field Answers. It shows packet sections as a polished decision-brief surface, connects each visible section to required data elements, shows answer/gap/source/review state, and exposes the actions already taken or still recommended to fill each element.
- **Evidence Store, Document Intake, PIID Profiles, SAM.gov Profiles, Capture Research, Quick Capture, and the LLM-wiki Knowledge Vault** are data, research, and knowledge inputs; they should feed the loop instead of remaining separate destinations.
...
**MVP-1B: Opportunity Activation + Packet Field Action Matrix + Opportunity Portfolio Foundation** ← **COMPLETE**
...
- Render the Living Packet as a visual live packet surface with packet sections plus required Packet Field Definitions, not only aggregate section counts or plain rows.
...
- Expose Packet Field Route Kinds so UI and API clients can distinguish source-backed answer extraction, research/MCP candidates, source-profile lookup, model synthesis, and customer-call-plan fallback rather than parsing route text.
...
**MVP-1C: LLM-Wiki Knowledge Vault Foundation** ← **COMPLETE**

Goal: make Ariadne's compounding knowledge brain real: one canonical Karpathy-style Obsidian/LLM-wiki vault that connects foundational knowledge, global data elements, source summaries, skills/capabilities, reusable insights, and typed relationships so Ariadne can execute opportunities more intelligently without making wiki synthesis the source of truth for opportunity-specific answers.

Deliverables:

- Establish the canonical local vault at `knowledge/ariadnes-thread/`, configured by `ARIADNE_OBSIDIAN_VAULT_DIR`, with `index.md`, `log.md`, an Ariadne wiki schema/instructions file, health/lint reports, proposals, generated projections, and stable folders for inbox, foundation, data elements, entities, relationships, sources, opportunities, workflows, skills/capabilities, reusable insights, and Hermes learning.
- Use the Karpathy LLM-wiki pattern deliberately: the vault is a persistent, compounding artifact maintained by the user, AI/LLM sessions, and later Hermes; it should update cross-links, source summaries, concept pages, relationship maps, contradictions, stale claims, and reusable insights over time.
- Define Ariadne page types and frontmatter for global data elements, capture concepts, source summaries, entities, relationships, workflow/capability pages, artifact patterns, source manifests, reusable insight candidates, opportunity projections, and Hermes learning proposals.
- Define a typed relationship vocabulary for lightweight knowledge graph links such as supports, answers, informs, blocks, contradicts, derived from, evidence for, fills gap in, suggests route, uses capability, applies to gate, produces artifact block, candidate reusable insight, expects data element, maps to artifact block, and uses source.
- Build the first global data-element pages for the Living Milestone Decision Briefing Packet and Packet Field Action Matrix so each data element explains why it matters, evidence standards, common source types, related workflows, related skills/capabilities, source refs, and how opportunity-specific answers should be managed.
- Migrate the full `docs/reference/project-ariadne/knowledge/` Markdown corpus into Ariadne-native vault pages and source summaries while preserving relative source paths to avoid collisions. Add normalized artifact/data-element expectation pages for template-like material, preserve provenance in frontmatter and a migration coverage report, and keep the old corpus until maintainer review explicitly approves retirement.
- Represent Project Theseus as complementary solicitation-parser/ontology capability context behind the Document Intake and Extraction Bundle boundary. Theseus can inform hot-button, evaluation-factor, requirement, and relationship candidates, but it must not become Ariadne's trusted opportunity record owner.
- Connect skills, vendored marketing capabilities, federal-data capabilities, source providers, document intake, artifact assembly, workflow routing, and future Hermes roles into the vault so Ariadne can choose better routes for packet fields, capture research, call-plan prep, action recommendations, and artifact drafts.
- Add the first vault health/lint concept: orphan pages, missing source refs, stale claims, conflicting claims, unsupported reusable insight candidates, unconnected data elements, and relationship pages with weak provenance.
- Keep opportunity-specific Packet Field Answers, accepted Evidence Items, Action Plan state, Capability Run Outputs, Artifact Block Reviews, review decisions, and trusted workflow records in Ariadne structured stores. The vault can project and discuss them, but changes to those records must return as Mirror Update Proposals.
- Allow selected vault pages and relationships to inform Opportunity Activation, packet-field route recommendations, Capture Research Briefs, call-plan prep, Next Action Recommendations, and Artifact Source Packages.
- Let Hermes participate as a future vault maintainer by writing proposals, lint findings, relationship suggestions, reusable-insight nominations, and learning notes without silently changing trusted opportunity execution records.

Acceptance demo:

- The user opens the Obsidian vault at `knowledge/ariadnes-thread/` and sees a useful `index.md`, append-only `log.md`, Ariadne schema instructions, and connected folders rather than an unstructured note dump.
- At least one global packet data element has a page connected to milestone gates, evidence standards, source types, source summaries, skills/capabilities, likely workflows, and reusable insight candidates.
- At least one source summary or concept page is migrated from the old Project Ariadne corpus into Ariadne-native shape with provenance, relationship links, and migration status.
- The migration coverage report shows what Project Ariadne material was incorporated, skipped, or still pending, and states that `docs/reference/project-ariadne/` remains temporary until maintainer retirement approval.
- A vault health/lint report identifies at least orphan or weakly sourced pages, missing source refs, and unconnected data elements.
- A selected vault page or typed relationship can inform an Opportunity Activation route or packet-field route recommendation without becoming the opportunity-specific Packet Field Answer.
...
**MVP-2: AI Usage Layer + Skills Integration** ← **IMPLEMENTED AND MERGED TO MAIN**
...
- MVP-2 may add deterministic skill-chain plan maps and short visible chain stages, but there is no LangGraph runtime adoption in this slice. Chain maps should stay inspectable, bounded, interruptible, and review-gated.
- Hermes may propose skill and chain improvements through human-reviewed Improvement Proposals, but it must not silently mutate skills, chains, or trusted workflow records.
- Preserve fake-runner tests, Capability Run provenance, approval prompts, source limitations, review destinations, and no automatic trusted downstream writes.
- Keep parser/RAG/graph/rendering expansion out of MVP-2 unless a later approved issue and `grill-with-docs` session changes that boundary; this means no parser/RAG/graph/rendering expansion while completing the AI/skills integration slice.
...
- MVP-2 merged to `main` on May 21, 2026. Validation: `uv run ruff check src tests` passed and `uv run pytest -q` passed with 402 tests.
...
- MVP-3 first tracer completed on May 22, 2026 via issues `#102`-`#106` and merged into the MVP-3 epic branch.
- Review-gated flow implemented end-to-end:
  - review-ready `competitive-gap-route-hint` output -> Work Product Deltas
  - packet delta accept/edit -> `PacketFieldAnswer` write + activation material refresh
  - action-plan implication accept/edit -> reviewable Next Action Recommendation
  - engagement-prep candidate creation from action link, packet gap, or fallback
  - artifact source-package context refresh from accepted packet/action deltas with freshness status surfaced in Artifacts mode
- Inspectability present in UI/API for source refs, assumptions, gaps, route/capability provenance, review decisions, and created/updated record surfaces.
- Boundaries verified: no renderer/export invocation, no automatic trusted writes on route/discard, no engagement-prep trusted write side effects.
- Validation for MVP-3 closure:
  - `uv run pytest -q tests/test_production_command_center.py` -> pass
...
- browse the Ariadne LLM-wiki Knowledge Vault and use a vault page, source summary, or typed relationship as context for a packet, call-plan, research, or action route.
...
**The next build direction is MVP-3: Capture Work Product Loop. Use the completed Opportunity Activation, Packet Field Action Matrix, Capability Run Store, AI/model contracts, focused skills, federal-data/source-provider capabilities, Artifact Assembly Source Packages, and LLM-wiki Knowledge Vault to route reviewed AI/skill/research outputs into the Living Milestone Decision Briefing Packet, Capture Action Plan, call/engagement prep, risk/follow-up candidates, and refreshed Artifact Draft context while preserving provenance, fake-runner tests, review gates, and no automatic trusted downstream writes.**
