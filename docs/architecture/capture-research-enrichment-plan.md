...
Date: 2026-05-18  
Status: selected through `grill-with-docs`; implementation pending
...
- `08-build/02-firecrawl-source-collection`
...
- Use **Firecrawl** as the first live Web Source Collection adapter in this build.
- Live Firecrawl runs are in scope when `FIRECRAWL_API_KEY` is configured and the run has explicit user approval or a future approved autonomy policy.
- Automated tests should use fake source-collection adapters and must never present fake output as live Firecrawl success.
- The first workflow should use a fixed product sequence rather than LangGraph, Hermes runtime, or general skill chaining.
- Future Hermes or agent-coordinated source collection should follow Graduated Autonomy rules: approved source scope, credit/budget boundaries, provenance, reviewability, and reversible downstream effects.
...
- Capability Runs can record execution/provenance detail for Firecrawl or skill execution, but the Capture Research Enrichment Store owns the product workflow meaning.
...
4. Run fake or live Web Source Collection. Live Firecrawl runs require configured credentials, bounded source targets, approval basis, and source provenance.
...
- `capability_run_refs` for Firecrawl or skill executions when applicable
- `review_decisions`
...
## Firecrawl And Source-Access Boundary

Live Firecrawl behavior is part of the first build, but it should be deliberate and traceable:

- Page render must not trigger live Firecrawl calls.
- User-triggered runs may call live Firecrawl when `FIRECRAWL_API_KEY` is configured.
...
- Live validation can be a separate local/manual check when the private key is configured.
...
- approval state for live Firecrawl collection;
...
- Use fake Firecrawl/source-collection adapters in normal automated tests.
...
- Keep optional live Firecrawl validation outside the normal unit-test path and require private local configuration.
...
3. If `FIRECRAWL_API_KEY` is configured and the user approves, Ariadne runs live Firecrawl against bounded public targets. If not, the demo can show fake-adapter behavior clearly marked as fake for development validation.
...
4. Add the Firecrawl adapter boundary and live readiness checks using `FIRECRAWL_API_KEY`.
...
10. Validate with `uv run ruff check src tests`, `uv run pytest -q`, and an optional live Firecrawl smoke check when configured.
